...
--
-- !! 적용 상태: 2026-08-19 커밋 시점 기준 prod·staging **미적용**.
--    manual-migrations.json 등재는 CI 게이트 통과를 위한 것이며, 실제 적용 후
--    이 주석 블록을 지울 것. prod 는 이미 같은 정의를 가지므로 no-op 이 예상되나,
--    적용 전에 prod 의 pg_get_functiondef 와 아래 본문을 대조해 확인해야 한다.
--    staging 은 lookup_contact_by_invite_token 의 anon/authenticated EXECUTE 가
--    남아 있어 이 파일의 REVOKE 가 실제로 권한을 조인다.
--
