...
- `InteractiveTableResponse`가 모바일 `original` 모드일 때 일반 행, 동적 행 확장, 가상화 행 모두에 셀 배경색을 적용하지 않도록 `applyCellBackground`를 전파했다.
- 100행 가상화 임계값을 넘는 모바일 original 표도 기존 배경을 보존하는 회귀 테스트를 추가했다.
