...
async function main() {
  ...
  const { findPrunableVersionIds } = await import(
    '@/server/survey-builder/services/versioning/version-retention-sql'
  );
  const { pruneVersionSnapshots } = await import('@/server/survey-builder/services/versioning/version-prune');
