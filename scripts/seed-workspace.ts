 ...
 *
 * 슈퍼어드민이 **먼저** 있어야 한다(`pnpm auth:seed`) — 팀 감사 행의 행위자이자 0106 백필이
...
async function main() {
  ...
  if (!DRY_RUN && !orgName) {
    ...
    // 지우지 못하고 종료만 된다(0110). 계획 출력에는 기본 이름을 써도 남는 것이 없다.
  ...
  console.log(
    `  배치 대기 내역 — 시드 슈퍼어드민 소유(0106 백필) ${summary.backfilledToSeed.length} / ` +
      `다른 소유자(해산 유래) ${summary.pendingWithOtherOwner.length}`,
  );

  for (const id of summary.pendingWithTeam) {
    console.error(`  계약 위반 — 배치 대기인데 팀이 있다 (0106 CHECK 확인): ${id}`);
  }
  for (const id of summary.assignedWithoutTeam) {
    console.error(`  계약 위반 — 배치됐는데 팀이 없다 (0106 CHECK 확인): ${id}`);
  }
  ...
  if (summary.missingOwner.length > 0) {
    ...
    if (!DRY_RUN) {
      // 0106 백필의 보정이다. **소유자 컬럼만** 건드린다 — assignment_status 를 함께 손대면
      // 이미 배치된 설문이 배치 대기로 되돌아간다(0106 헤더의 경고).
