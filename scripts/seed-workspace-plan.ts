...
/**
 * 팀 5개 (PRD 완료 정의). 이름은 전체 조직 경로를 포함한다(0115 헤더 규약).
...
export interface SurveyBackfillSummary {
  ...
  pending: number;
  /**
   * 소유자가 비어 있는 설문 — 0116 백필이 슈퍼어드민보다 먼저 돈 흔적이다. **이 티켓이
   * 실제로 재는 것은 이 하나뿐**이고, 나머지는 분류이거나 이중 확인이다.
   */
  missingOwner: string[];
  /** 배치 대기 + 소유자가 시드 슈퍼어드민 — 0116 백필이 실제로 닿은 행. */
  ...
  /**
   * 0116 의 `surveys_assignment_check` 와 같은 조건의 **이중 확인**이다. 그 CHECK 가 살아 있는
 ...
 * 슈퍼어드민」을 실제 행으로 되짚는다.
 *
 * **세 조건의 무게가 서로 다르다.** 앞의 둘은 0116 의 validated CHECK 가 DB 에서 이미 강제하므로
 * 여기서는 이중 확인일 뿐이고, 실제로 어긋날 수 있는 것은 **소유자**다 — 0116 의 백필은 그
