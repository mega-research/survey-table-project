...
1. `기본형`
   ...
   - 기존 화면의 픽셀 보존을 목표로 하지 않는다. 로고가 없는 설문의 새 기본 표현이다.
...
기존 설문 row와 기존 배포 스냅샷에는 `responseHeader`가 없다. 이 경우 `{ style: 'plain', titleSize: 'auto' }`로 취급한다. 기존 응답 페이지와 픽셀 단위로 동일할 필요는 없고, 새 기본형 헤더가 자연스럽게 적용되면 된다.
...
- 헤더 로고 이미지는 설문 이미지 자산으로 취급한다. 업로드 직후의 `tmp/survey/` URL은 저장 서비스 경계에서 기존 질문 이미지와 같은 방식으로 영구 `survey/` URL로 승격한 뒤 DB와 배포 스냅샷에 남긴다.
...
- 헤더 로고 `imageUrl`의 `tmp/survey/` URL이 저장 전에 영구 `survey/` URL로 승격되는 단위 테스트를 추가한다.
