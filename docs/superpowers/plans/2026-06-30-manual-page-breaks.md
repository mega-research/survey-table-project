...
| `src/lib/versioning/snapshot-builder.ts` | 스냅샷 쓰기 타입(`SnapshotQuestion`) + 매핑 | 1 |
| `src/db/schema/schema-types.ts` | 스냅샷 읽기 타입(`QuestionData`) — page-dwell가 읽음 | 1 |
...
| `tests/unit/survey/survey-step-question-number-badges.test.tsx` | 재작성(삭제 뷰 의존 제거) | 3 |
...
- [ ] **Step 8b: 스냅샷 읽기 타입(`QuestionData`)에도 추가**

`src/lib/operations/page-dwell.ts`는 스냅샷을 `SurveyVersionSnapshot`(schema-types.ts)으로 읽는다. 이 타입의 질문은 `snapshot-builder`의 `SnapshotQuestion`과 **별개의 손유지 타입** `QuestionData`다. Task 4가 `q.pageBreakBefore`를 읽으려면 여기에도 필드가 있어야 한다.

`src/db/schema/schema-types.ts`의 `QuestionData` 인터페이스 `hideTitle?: boolean;`(line 188) 뒤에 추가:
```ts
  pageBreakBefore?: boolean;
```
...
- Test: `tests/unit/survey/page-step-view.test.tsx` (신규), `tests/unit/survey/survey-step-question-number-badges.test.tsx` (재작성 — 삭제 뷰 의존 제거)
...
- [ ] **Step 9: 숫자 배지 테스트 재작성 (삭제 뷰 의존 제거)**

`tests/unit/survey/survey-step-question-number-badges.test.tsx`는 삭제될 `GroupStepView`/`TableStepView`와 구 `StepItem` 모양을 쓴다. 의도("응답 스텝에 숫자 원형 배지를 렌더하지 않는다")를 유지하며 전체 교체:
...
- [ ] **Step 11: 응답 측 타입/테스트 확인**

Run: `pnpm test page-step-view group-ordering survey-step-question-number-badges`
Expected: PASS. 이어서 응답 플로우에 잔여 `currentStep.kind`/`step.question` 참조가 없는지 grep으로 확인: `grep -n "\.kind\|step.question" src/components/survey-response/survey-response-flow.tsx` 결과가 비어야 한다. (운영 콘솔 `profiles.ts`/`page-dwell.ts`는 Task 4 전까지 tsc 에러가 남아있을 수 있음 — 예상된 상태.)
...
- [ ] **Step 5: `renderEditCard`에만 인서터 배치 (DragOverlay 제외)**

`sortable-question-list.tsx`에는 `SortableQuestion` 렌더가 2곳 있다 — `renderEditCard`(line ~743, 편집 카드)와 `renderDragOverlay`(line ~842, **드래그 미리보기**). divider는 **`renderEditCard`에만** 넣는다(오버레이에는 넣지 않는다).

첫 질문 억제는 배열 인덱스가 아니라 **선형 렌더 순서의 첫 질문 id**로 판단한다(`questions.indexOf`는 렌더 순서와 다를 수 있음). 컴포넌트 본문 상단에서 한 번 계산:
...
`renderEditCard`의 반환 JSX에서 래퍼 `<div ...>` 바로 안쪽, `<SortableQuestion ...>` **앞에** 추가:
...
`updateQuestion`/`groups`/`questions`는 이 컴포넌트가 이미 받는 props/store 값이다. `updateQuestion`이 스코프에 없으면 `const { updateQuestion } = useSurveyStore();`로 가져온다(파일이 이미 `useSurveyStore`를 쓰는지 먼저 확인).

> 근거: `renderDragOverlay`(line ~842)는 드래그 중 떠다니는 카드 미리보기라 페이지 구분점 UI가 들어가면 안 된다. 엔진(Task 2)이 선형 첫 질문의 `pageBreakBefore`를 무시하므로, UI에서도 첫 질문 위 divider를 숨겨 "켰는데 아무 일도 안 일어나는" 혼란을 막는다.
