...
- 파싱 전에 모든 공백을 제거하므로 `2 - 3`은 `2-3`, `1 2`는 `12`로 해석한다.
- PostgreSQL integer 상한을 위한 추가 max 검증은 두지 않는다. 상한 초과 저장 오류는 운영자 테스트 책임으로 수용한다.
...
- 필드가 모두 없는 과거 질문은 기존 표시를 보존해 `hideColumnLabels=true`이면 `null/null`, 그 외에는 `0/0`으로 해석한다. 명시적 `null/null`은 반복 없음이다.
- 음수·비정수·역전 범위·한쪽만 null/undefined인 저장값은 `0/0`으로 폴백하고 본문 행을 목차에서 제거하지 않는다.
- 반복 본문 행은 현재 표시되는 경우에만 상세에 나오며 목차와 전체·섹션 진행률에서 제외한다.
- 반복 행에 입력 셀이 있어도 별도 경고나 예외를 만들지 않고 응답값 없는 기존 모양의 비활성 컨트롤로 렌더한다. 기존 응답 데이터는 삭제하지 않는다.
- 반복 행의 비활성 처리는 해당 모바일 상세에만 적용하며 공용 PreviewCell의 기존 미리보기 모양을 바꾸지 않는다.
- 반복 텍스트 셀의 컨택 속성 토큰은 선택 응답 행과 동일하게 치환한다.
...
- 동적 생성 행, 비연속 범위, 섹션별 범위, 입력행 안전 검사, 필수·조합 검증 예외를 추가하지 않는다. 반복된 필수 입력으로 완료가 막히는 위험은 운영자 검수로 수용한다.
- choice 설명 표에는 기존에 없는 행 displayCondition 평가를 새로 추가하지 않는다. 조건부로 숨은 반복 행 생략은 현재 조건 평가를 지원하는 일반 table에만 적용한다.
- 질문 편집 모달의 `Ctrl/Cmd+S` 저장과 `Escape` 닫기를 제거하고 Radix 기본 Escape 닫기도 막는다. 저장은 버튼으로만 실행하며 X·취소는 유지한다.
...
- `src/lib/question/normalize.ts`: 반복 헤더 두 필드의 의미 있는 null을 strict 정규화에서도 보존.
...
- `src/features/library/server/services/saved-questions.service.ts`: 과거 보관 질문의 hideColumnLabels 호환 canonicalize와 명시적 null 보존.
...
- `src/components/survey-builder/question-edit-modal.tsx`: store-only 두 필드의 저장·취소·null 보존과 저장·닫기 키보드 단축키 제거.
- `src/components/survey-builder/table-preview.tsx`: row-aware render override, non-sticky 옵션, 선택적 행 높이 보존, 중복 셀 key 분리.
- `src/components/survey-builder/cells/preview-cell.tsx`: 반복행에서만 checkbox/radio 컨트롤을 비활성화하는 opt-in prop.
- `src/utils/mobile-original-row.ts`: 반복 본문 블록 투영, 헤더 유무 판정, rowspan 경계 분리.
- `src/components/survey-builder/mobile-original-row-table.tsx`: 여러 행 중 선택 행만 인터랙티브 렌더하고 반복 텍스트 토큰 치환.
...
- `tests/unit/survey/preview-cell-choice.test.tsx`: 공용 PreviewCell 기본 모양과 반복 행 전용 disabled opt-in 회귀.
...
- Modify: `src/lib/question/normalize.ts:34-65`
...
`normalize.ts`의 최상위 null 제거에는 두 의미 있는 null 키만 보존하는 예외를 추가한다. 다른 nullable DB 컬럼의 기존 수렴 동작은 유지한다.
...
```bash
git add src/utils/mobile-drilldown-repeat-header.ts src/types/survey.ts src/db/schema/schema-types.ts src/lib/question/variants.ts src/lib/question/schema.ts src/lib/question/normalize.ts tests/unit/utils/mobile-drilldown-repeat-header.test.ts tests/unit/question/schema-matrix.test.ts tests/unit/question/normalize.test.ts
...
`tests/unit/features/library/apply-multiple-questions-order.test.ts`에 세 케이스를 추가한다.
...
it('열 제목을 숨기던 과거 모바일 표 질문은 반복 없음으로 canonicalize한다', async () => {
  findMany.mockResolvedValue([{
    id: 'legacy-hidden-label-repeat-header',
    question: {
      id: 'legacy-hidden-table',
      type: 'table',
      title: '과거 숨김 표',
      required: false,
      order: 0,
  ...
  const [applied] = await applyMultipleSavedQuestions(['legacy-hidden-label-repeat-header']);
...
두 컬럼에 `NOT NULL`이나 CHECK를 추가하지 않는다. 신규·일반 기존 질문은 default `0/0`을 사용하고, 기존 `hideColumnLabels=true` 질문만 배포 전 숨김 표시를 보존하도록 `null/null`로 이관한다. 손상값 폴백은 읽기 유틸리티 책임이다.
...
이 코드는 missing old snapshot을 `hideColumnLabels=true`이면 `null/null`, 그 외에는 `0/0`으로 만들고 explicit off를 `null/null`로 유지한다.
...
- [ ] **Step 5: 편집 모달의 취소·저장에서 두 필드와 null을 보존하고 키보드 저장·닫기 제거**
...
`question-edit-modal.tsx`의 document `keydown` 등록과 `handleKeyDown` callback 전체를 삭제한다. 헤더의 `저장: Ctrl+S`, `닫기: ESC` 안내 블록도 삭제한다. Radix Dialog가 기본으로 Escape를 처리하므로 `DialogContent`에 다음을 추가한다. X 버튼과 취소 버튼 경로는 바꾸지 않는다.
...
기존 “저장 중 Escape 무시”, “저장 중 Ctrl+S 중복 방지” 테스트는 키보드가 idle 상태에서도 아무 동작을 만들지 않는 다음 테스트로 교체한다. 기존 저장 버튼·취소·X 롤백 테스트는 그대로 유지한다.
...
저장은 버튼 클릭만 사용한다. 범위 입력에 포커스가 있으면 브라우저 blur가 먼저 실행되어 `commitRepeatHeaderDraft`가 store를 갱신한 뒤 저장 버튼 handler가 실행되는지 사람 검수 체크포인트에서도 확인한다.
...
Expected: 범위 수명주기와 저장 버튼·X·취소 PASS, `Ctrl/Cmd+S`·Escape 무동작 PASS, TypeScript 오류 0개.
...
- Modify: `tests/unit/survey/preview-cell-choice.test.tsx`
...
`tests/unit/survey/mobile-original-row-table.test.tsx`에 `ContactAttrsProvider`를 import하고 다음 렌더 테스트를 추가한다.
...
- [ ] **Step 5: PreviewCell에 모바일 반복 행 전용 disabled opt-in 추가**

`PreviewCell`의 현재 inline props 타입에 `disableControls?: boolean`을 추가하고 함수 destructuring 기본값에 `disableControls = false`를 추가한다. 기존 빌더 미리보기는 지금처럼 readOnly 모양을 유지하고 모바일 반복 행만 실제 disabled가 된다.
...
`tests/unit/survey/preview-cell-choice.test.tsx`에는 기본 PreviewCell이 enabled 모양을 유지하고 opt-in에서만 disabled가 되는 회귀 테스트를 추가한다.
...
`mobile-original-row-table.tsx`에 기존 기본 셀 렌더러와 컨택 토큰 도구를 가져온다.
...
const renderMobileCell = useCallback(
  ...
  [attrs, choiceControlType, interactiveRowId, renderCell],
);
```

반복행 PreviewCell에는 응답 store 값을 전달하지 않는다. 따라서 기존 응답 데이터는 유지되지만 반복 헤더에는 빈 input·미선택 control의 기본 모양만 표시된다.
...
Expected: 반복행 상세 한정 disabled·토큰 치환·rowspan split·header fallback PASS, 공용 PreviewCell과 기존 원본 표 회귀 PASS, TypeScript 오류 0개.

- [ ] **Step 8: Task 4 커밋**

```bash
git add src/utils/mobile-original-row.ts src/components/survey-builder/table-preview.tsx src/components/survey-builder/cells/preview-cell.tsx src/components/survey-builder/mobile-original-row-table.tsx tests/unit/utils/mobile-original-row.test.ts tests/unit/survey/mobile-original-row-table.test.tsx tests/unit/survey/preview-cell-choice.test.tsx
...
`rows`는 `question.tableRowsData` 원본을 그대로 사용한다. choice 응답 렌더러에는 현재 행 displayCondition 평가가 없으므로 이 Task에서 `useDynamicRows`, branch evaluator 또는 별도 조건 필터를 추가하지 않는다. 조건부 반복행 생략 테스트는 일반 table Task 5가 소유한다.
...
pnpm exec vitest run \
  ...
  tests/unit/survey/question-edit-modal-mobile-display.test.tsx \
...
pnpm exec prettier --check \
  ...
  src/components/survey-builder/question-edit-modal.tsx \
  ...
  docs/superpowers/specs/2026-07-22-mobile-drilldown-repeated-header-rows-design.md \
...
- [ ] **Step 9: 병합·원격 마이그레이션 전 사람 모바일 검수**

로컬 Supabase migration과 dev server를 실행한 상태에서 일반 table, radio 설명 표, checkbox 설명 표 fixture를 각각 만든다. 일반 table fixture에는 앞쪽 제목 열, 다단 헤더, hidden 셀, colspan, 반복/응답 경계를 넘는 rowspan, 컨택 속성 토큰, 필수 input을 포함한다.

각 fixture에서 다음을 실제 모바일 viewport로 확인한다.

1. 빈 값, `0`, `3`, `2-3`, `0-2`의 헤더·반복행·선택행 순서.
2. 앞쪽 열 제외 뒤 정식 헤더·반복 본문·선택 행의 열 정렬.
3. hidden, colspan, rowspan, 행 높이와 일반 table 조건부 숨김 반복행 생략.
4. 반복 input·radio·checkbox는 응답값 없이 비활성이고 선택 행만 입력 가능함.
5. 반복 텍스트의 컨택 속성 토큰 치환.
6. 항목 간 이동 시 가로 위치 유지와 목차 복귀 시 0 초기화.
7. 반복 범위에 필수 input을 넣으면 완료가 막히는 합의된 위험을 확인하고 실제 운영 fixture에서는 해당 행을 범위에서 제거함.
8. `Ctrl/Cmd+S`와 `Escape`가 질문 모달을 저장·닫지 않으며 저장 버튼은 포커스된 범위 입력을 blur 확정 후 저장함.
9. X·취소는 저장하지 않고 원래 반복 범위로 롤백함.
10. 반복 필드가 없던 기존 질문은 `hideColumnLabels=true`일 때 헤더 숨김을 유지하고, false일 때 기존 헤더를 유지함.

Expected: 사용자가 위 항목을 모두 승인하기 전에는 브랜치를 병합하지 않고 공유 Supabase 원격에 migration을 적용하지 않는다.
...
| 모든 공백 제거·PostgreSQL integer max 검증 미추가 | Task 1 |
| 과거 missing의 hideColumnLabels 호환, explicit null/null, 손상값 0/0 폴백 | Task 1, Task 2 |
| strict 정규화의 의미 있는 반복 헤더 null 보존 | Task 1 |
| out-of-range 무보정·현재 작성 위치 | Task 1 |
| 질문 생성·수정·복제·전체 저장·보관함·스냅샷 | Task 2 |
| 해당 모드에서만 입력, Enter/blur commit, 모드 전환 보존 | Task 3 |
| Ctrl/Cmd+S·Escape 제거, 저장 버튼 전용, X·취소 유지 | Task 3 |
| 반복 본문 행 목차·진행률 제외 | Task 1, Task 5, Task 6 |
| 일반 table 조건부 숨김 행 생략·rowspan anchor 승격 | Task 1, Task 5 |
| choice table에 새 행 조건 평가를 추가하지 않음 | Task 6 |
| 정식 다단 헤더·열 label 폴백·둘 다 없을 때 0 생략 | Task 4, Task 5, Task 6 |
| 앞쪽 열 제외·colspan·rowspan·hidden | Task 4 |
| 반복 입력 무응답 기본 모양·상세 한정 disabled·선택 행만 응답 | Task 4 |
| 기존 응답 데이터 보존·공용 PreviewCell 모양 무변경 | Task 4 |
| 반복 필수 입력 검증 위험을 예외 없이 운영자 검수로 수용 | Task 4, Task 6 Step 9 |
| 반복 텍스트 컨택 토큰 치환 | Task 4 |
...
| 일반 table과 choice table 동일 범위·목차·상세 동작 | Task 5, Task 6 |
| 모든 행 반복 시 빈 목차 0/0 | Task 5 |
| 도메인 언어 갱신 | Task 6 |
| 병합·원격 migration 전 사람 모바일 검수 | Task 6 Step 9 |
