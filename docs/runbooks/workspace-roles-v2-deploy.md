...
- **0101 은 적용 금지.** Better Auth 5테이블은 프로덕션·스테이징에 2026-07-14 선반영돼 있다.
  이 파일은 빈 DB 재생(`pnpm db:setup-test`)과 신규 환경 부트스트랩 전용이다.
- 적용 대상은 **0102~0112 열한 개**다.
- 0112(`survey_responses.fieldwork_user_id`)까지 끝난 뒤 앱을 배포한다. 컬럼 추가는 전부
...
### 순서가 실제로 중요한 자리: 0106 백필

`0106_surveys_team_scoping.sql` 은 기존 설문 전부를 `team_id=NULL` +
...
- 만드는 팀: `연구1본부 - 1팀` · `연구2본부 - 3팀` · `연구3본부 - 5팀` · `연구3본부 - 6팀` ·
  `연구3본부 - 7팀`. 이름은 전체 조직 경로를 포함한다(0105 규약).
...
나와 56건이 되는데, 그것이 바로 이 브랜치의 `0111` 이 `public.users` 로 옮기는 대상이다
(적용 전이라 정상). 반대 방향 「레포에만 있음」 109건은 미적용 0102~0112 그대로다 — prod
대조에서만 함께 보고된다.
...
- 스키마 롤백은 없다. 0102~0112 는 컬럼·테이블 **추가**라 앱만 되돌리면 이전 코드가 그대로
...
- 리허설: 프로덕션 스냅샷을 스테이징에 복원 → 0102~0112 적용 → §4 시드 → §7 스모크.
...
| 기존 설문 백필 | 0106 이 하고 시드가 검증·보정 (§1·§4) |
