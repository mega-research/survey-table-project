...
export const questions = pgTable(
  ...
  (table) => [
    check(
      ...
      sql`${table.mobileTableDisplayMode} in ('auto', 'drilldown-original-row', 'row-wise-original', 'row-cards', 'row-group-cards', 'original')`,
    ),
    check(
...
export const responseEditLogs = pgTable('response_edit_logs', {
  ...
  contactTargetId: uuid('contact_target_id'),
  action: text('action').$type<'edit' | 'reset' | 'reedit_allow'>().notNull().default('edit'),
  surveyId: uuid('survey_id')
    .notNull()
    .references(() => surveys.id, { onDelete: 'cascade' }),
  ...
  changedQuestions: jsonb('changed_questions').$type<ResponseEditChange[]>().notNull().default([]),
