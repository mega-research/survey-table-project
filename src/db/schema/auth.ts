 ...
 *
 * 프로덕션·스테이징 DB 에는 5테이블이 선반영돼 있다(0111 재생용 마이그레이션 참조).
 * better-auth 1.7 어댑터 기대와의 차이(accounts.issuer 등)는 0112 가 해소한다.
 */
export const users = pgTable('users', {
  ...
  // 소속 기관 메모 — guest 전용 자유 입력 (0113). internal 은 팀 멤버십(티켓 06),
  // fieldwork 는 실사 업체 엔티티(티켓 24)에서 소속을 얻으므로 이 컬럼을 쓰지 않는다.
  organization: text('organization'),
  // 계정 유형 — internal | guest | fieldwork (ADR-0018, 0112 마이그레이션)
  userType: text('user_type').$type<UserType>().notNull().default('internal'),
  // 소속 실사 업체 — fieldwork 전용 (0120, 티켓 24).
  //
  // FK 는 0120 마이그레이션의 ALTER TABLE 이 만든다. drizzle 에서 `.references()` 를 붙이지
  ...
  // 업체 내 역할 — leader | worker. fieldwork 전용 (0120).
  //
  ...
  // 세션이 마지막으로 일괄 폐기된 시각 (0114, 티켓 30). 재설정·상태 전이가 갱신한다.
