...
/**
 * 순위형(ranking) 질문 응답 컴포넌트 — 보기를 눌러 순위를 매긴다.
 ...
 * - allowDuplicateRanks: 클릭으로는 같은 보기를 두 순위에 둘 수 없어 드롭다운 스택을 유지
 */
...
// ── 드롭다운 폴백 (allowDuplicateRanks) ────────────────────────────────────

interface RankingDropdownFallbackProps {
  question: Question;
...
/**
 * 중복 순위 허용 문항의 드롭다운 스택 — 클릭 방식은 같은 보기를 두 순위에 둘 수 없다.
 * 옛 응답 UI 를 그대로 옮겨 둔 것이다.
 */
function RankingDropdownFallback({
  question,
  rawOptions,
  answers,
  groupedMap,
