 ...
 * admin 레이아웃(guestPathRedirect)이 여기로 보낸다. 안내 페이지를 거치지 않고
 * 즉시 세션을 끝내 로그인 화면으로 보낸다 — 다른 설문을 보려면 해당 설문
 * 담당 계정으로 다시 로그인하라는 운영 결정(2026-08-14).
 *
 * GET 인 이유: 레이아웃 redirect 는 GET 으로만 이어질 수 있다. GET 상태 변경의
 * CSRF 리스크는 두 겹으로 좁힌다 — 게스트가 아니면 signOut 없이 돌려보내고
 * (관리자 대상 로그아웃 CSRF 차단), Better Auth signOut 은 현재 세션 토큰만
 * 폐기한다(공유 게스트 계정의 타 기기 실사 세션 보존).
 */
export async function GET(request: Request) {
  // 레이아웃이 실어 보낸 원래 목적지와 사유를 로그인창까지 전달 — 재로그인 시 그
  // 설문으로 복귀시키거나 담당 계정이 아니라는 안내를 띄우는 근거가 된다.
  const params = new URL(request.url).searchParams;
  const redirectParam = sanitizeInternalPath(params.get('redirect'));
  const reasonParam = params.get('reason');
  const loginUrl = new URL('/admin/login', request.url);
  if (redirectParam) loginUrl.searchParams.set('redirect', redirectParam);
  if (reasonParam) loginUrl.searchParams.set('reason', reasonParam);

  // 실제 사용자 내비게이션이 아닌 요청(링크 prefetch, 백그라운드 fetch)은 상태
  // 변경 없이 돌려보낸다 — 레이아웃의 로그아웃 리다이렉트를 prefetch 가 따라와
  ...
  // 세션 쿠키 만료 지시는 signOut 응답의 Set-Cookie 에만 실린다 — 리다이렉트 응답으로
  // 옮겨 담지 않으면 쿠키가 남아 다음 요청이 다시 게스트 세션으로 들어온다.
