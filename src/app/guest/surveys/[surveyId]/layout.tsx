 ...
 * 관문을 갖는 것과 같은 구조다.
 *
 * 카드 머리(제목·기간·상태)는 홈과 **같은 read-model** 에서 가져온다. 설문 행을 따로 읽으면
 * 「홈에는 종료인데 열람 화면에는 진행중」 같은 어긋남이 생긴다 — 상태 판정의 정본이 둘이
 * 되기 때문이다.
 */
export default async function GuestSurveyLayout({ params, children }: Props) {
  ...
  const card = (await listGuestSurveys(user.id)).find((row) => row.surveyId === surveyId);
