...
export function accountHomePath(userType: UserType): string {
  return ACCOUNT_HOME_PATH[userType];
}
 ...
 * 다음 문자가 `/` 일 때만이다.
 */
export function isAccountTypePath(pathname: string, userType: UserType): boolean {
  ...
  if (userType === 'internal') return false;
 ...
 */
export function resolvePostLoginDestination(userType: UserType, target: string): string {
