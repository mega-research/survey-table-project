 ...
 *      /i/__test__ 는 미리보기/테스트발송용 placeholder 코드로 실제 컨택에 매칭되지 않아,
 *      클릭 시 not-found(초대 링크가 유효하지 않음)로 처리된다. 테스트 발송은 본문 렌더
 *      확인이 목적이라 무해.
 */
...
interface CampaignSendInput {
  ...
  /** 실제 invite URL — baseUrl + /i/{inviteCode}. 호출자가 buildInviteUrl 로 빌드해서 전달. */
  inviteUrl: string;
}
