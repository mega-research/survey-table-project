...
describe('useCellResponse — 미리보기 원본 주입 (test-response-store adapter)', () => {
  it('store 의 해당 셀 값을 읽는다', () => {
    useTestResponseStore.getState().updateTestResponse('q1', { c1: '저장값' });
    const { result } = renderHook(() => useCellResponse('q1', 'c1'), { wrapper: preview });
  ...
  it('updateValue: sibling 셀 응답을 빈값으로 클리어한 뒤 자신을 기록', () => {
    ...
    const { result } = renderHook(() => useCellResponse('q1', 'c1', undefined, undefined, ['c2', 'c3']), {
      wrapper: preview,
    });
...
describe('useCellResponse — 원본 없음 (value/onChange adapter)', () => {
  it('externalValue 의 해당 셀 값을 읽는다', () => {
    const { result } = renderHook(() => useCellResponse('q1', 'c1', { c1: '외부값' }));
  ...
  it('updateValue: 최신 externalValue 에 병합해 onChange 로 전달', () => {
    ...
    const { result } = renderHook(() =>
      useCellResponse('q1', 'c1', { c0: '기존' }, onChange),
    );
  ...
  it('updateValue: rerender 로 갱신된 최신 externalValue 를 병합 (stale closure 방지 ref 패턴)', () => {
    ...
    const { result, rerender } = renderHook(
      ({ value }: { value: Record<string, unknown> }) =>
        useCellResponse('q1', 'c1', value, onChange),
  ...
  it('updateValue: sibling 클리어가 실응답 경로에도 적용', () => {
    const onChange = vi.fn();
    const { result } = renderHook(() =>
      useCellResponse('q1', 'c1', { c2: '경쟁값' }, onChange, ['c2']),
    );
  ...
  it('onChange 미제공 시 커밋은 no-op 이지만 로컬 상태는 즉시 반영', () => {
    const { result } = renderHook(() => useCellResponse('q1', 'c1', {}));
...
describe('useCellResponse — 로컬 상태 동기화', () => {
  it('updateValue 직후 cellResponse 가 동기 반영된다 (UI 즉시 갱신)', () => {
    const onChange = vi.fn();
    const { result } = renderHook(() => useCellResponse('q1', 'c1', {}, onChange));
