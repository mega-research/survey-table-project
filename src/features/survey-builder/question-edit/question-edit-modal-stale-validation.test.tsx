...
vi.mock('@/shared/lib/rpc', () => ({
  client: {},
  // staging 의 모달은 셀 보관함 훅(orpc.library.savedCells) 을 렌더 시점에 구성한다 — 모양만 있는 스텁
  orpc: {
    library: {
      savedCells: {
        list: { queryOptions: () => ({ queryKey: ['savedCells'], queryFn: async () => [] }) },
        search: { queryOptions: () => ({ queryKey: ['savedCells', 'search'], queryFn: async () => [] }) },
        create: { mutationOptions: (o: object) => ({ mutationFn: async () => ({}), ...o }) },
        remove: { call: async () => ({}) },
        key: () => ['savedCells'],
      },
    },
...
function failSaveOnBrokenRadio() {
  const { rerender } = render(<QuestionEditModal questionId="qR" isOpen={true} onClose={() => {}} />, { wrapper });
...
// staging 의 QuestionEditModal 은 셀 보관함 저장 훅(TanStack) 을 쓴다 — 테스트도 QueryClient 가 필요하다
