/**
 * 보기-소스 표(ChoiceTableResponse)의 모바일 카드·행 카드·그룹 카드·축 카드 렌더.
 * SUT 는 question-renderer 지만 사이드카 정리 검증이 응답 저장소(useSurveyResponseStore)를
 * 직접 보므로 저장소를 소유한 survey-response 에 둔다(ADR 0017).
 */
  ...
  it('다음을 누른 뒤 미충족 필수 그룹의 섹션만 붉게 두른다', () => {
    ...
    render(
      <ChoiceTableResponse
        ...
        value={{ rad01: 'r1c1' }}
        onChange={() => {}}
        unfilledGroupCellIds={collectUnfilledChoiceGroupCellIds(q, { rad01: 'r1c1' })}
      />,
    );
  ...
  it('보기를 해제하면 카드에 셀이 없어도 사이드카 값이 지워진다 — 재선택에 이전 값이 되살아나지 않는다', () => {
    ...
    render(
      <ResponseSourcesProvider sources={liveResponseSources}>
        <ChoiceTableResponse question={gatedQuestion()} value={[]} onChange={() => {}} />
  ...
  it('다음을 누른 뒤 답하지 않은 필수 그룹의 타일만 붉게 두른다', () => {
    render(
      <ChoiceTableResponse
        ...
        value={{ chk1: ['r1c1'] }}
        onChange={() => {}}
        unfilledGroupCellIds={collectUnfilledChoiceGroupCellIds(twoGroupQuestion(), {
          chk1: ['r1c1'],
        })}
      />,
    );
  ...
  it('다음을 누른 뒤 미충족 필수 그룹의 카드만 붉게 두르고 그 그룹의 필수 문구를 머리에 붙인다', () => {
    render(
      <ChoiceTableResponse
        ...
        value={{ cb1: ['r1c2'] }}
        onChange={() => {}}
        unfilledGroupCellIds={collectUnfilledChoiceGroupCellIds(axisQuestion(), {
          cb1: ['r1c2'],
        })}
      />,
    );
  ...
  it('그룹 문구가 없으면 기본 필수 문구로 폴백하고, 다음을 누르기 전에는 붙지 않는다', () => {
    ...
    render(
      <ChoiceTableResponse
        ...
        value={{ cb2: ['r3c3'] }}
        onChange={() => {}}
        unfilledGroupCellIds={collectUnfilledChoiceGroupCellIds(q, { cb2: ['r3c3'] })}
      />,
    );
