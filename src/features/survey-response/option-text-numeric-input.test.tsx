...
describe('OptionTextInput — 숫자 모드', () => {
  ...
  it('숫자가 아닌 입력은 무시된다', async () => {
    ...
    renderLive(<OptionTextInput questionId="q1" option={numericOption} />);
  ...
  it('숫자 모드가 아니면 종전대로 자유 텍스트', async () => {
    const user = userEvent.setup();
    renderLive(<OptionTextInput questionId="q1" option={{ id: 'o2' }} />);
...
describe('OptionTextInput — 환산 읽기 표시', () => {
  ...
  it('값을 넣으면 단위 환산이 아래 줄에 뜬다', async () => {
    const user = userEvent.setup();
    renderLive(<OptionTextInput questionId="q1" option={wonOption} />);
...
describe('OptionTextInput — 칩 셸 모드', () => {
  ...
  it('안내가 셸(label) 바깥에 렌더된다', async () => {
    const user = userEvent.setup();
    renderLive(<OptionTextInput questionId="q1" option={wonOption} rowLabel="① 매출액" />);
