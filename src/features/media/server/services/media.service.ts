 ...
 * 실제 R2 삭제는 lib deleteImagesFromR2Server 에 위임하고, 여기서는 어떤 URL 이
 * 우리 R2 소유였는지(deletedUrls)와 외부 URL(skip)을 분리해 카운트만 재구성한다.
 */
export async function deleteImages(
  ...
  // 우리 R2 public URL 만 삭제 대상(외부 URL 은 skip). lib 와 동일 규칙.
  ...
  // 실제 R2 삭제는 lib 에 위임.
  const ok = await deleteImagesFromR2Server(urls);
