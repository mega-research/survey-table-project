...
import type { MatchContactUploadResult } from '@/shared/contracts/contacts-io';
  ...
  return (
    <details className="rounded border border-slate-200 bg-white px-3 py-2 text-xs">
      <summary className="cursor-pointer font-medium text-slate-700">
        {title} {total.toLocaleString('ko-KR')}건
        {total > samples.length && ` (샘플 ${samples.length}건 표시)`}
      </summary>
      <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto text-slate-600">
        {samples.map((s) => (
          <li key={s.excelRow}>
            {s.excelRow}행 —{' '}
            {Object.entries(s.keyValues)
              .map(([k, v]) => `${k}: ${v || '(빈 값)'}`)
              .join(' / ')}
          </li>
        ))}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {[
          ...
          {
            label: '다중 일치·키 빈 값',
            value: result.multiMatches + result.emptyKeys,
            tone: 'text-amber-600',
          },
        ].map((c) => (
      ...
      {!isMerge && result.fileDuplicates + result.emptyKeys > 0 && (
        <div className="text-xs text-slate-500">
          파일 내 키 중복·키 빈 값 행은 서로 간 중복 검사 없이 추가되며, 기존 명단과 일치하는 행은
          위 선택을 따릅니다.
        </div>
      )}

      <SampleList
        title={isMerge ? '불일치 행' : '신규 행'}
        samples={result.unmatchedSamples}
        total={result.unmatched}
      />
      <SampleList
        title="파일 내 키 중복 행"
        samples={result.fileDuplicateSamples}
        total={result.fileDuplicates}
      />
      <SampleList
        title="다중 일치 행"
        samples={result.multiMatchSamples}
        total={result.multiMatches}
      />
