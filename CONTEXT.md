...
Raw Data 시트와 분할 시트 왼쪽에 붙는 운영 메타 열 묶음 — 번호(resid)·순번·조사 대상 그룹·개별 URL·IP 해시·상태·시작일시·종료일시·소요시간·마지막 입력 문항·device. 번호와 순번은 모드 분기 없이 항상 함께 나간다. SPSS 변수가 아니므로 코딩북·.sav에는 포함하지 않으며 헤더는 3행 세로 병합이다.
...
진행 위치(currentStepId, 페이지 단위)를 그 페이지 대표(첫) 질문의 질문번호(Qx)로 환산한 표시값 — 운영 콘솔 진척 표기와 동일 기준. 질문 단위 정밀도가 아니라 페이지 단위다.
_Avoid_: 마지막 응답 질문(질문 단위 어감)
