...
진행 위치(currentStepId, 페이지 단위)를 그 페이지 대표(첫) 질문의 질문코드(questionCode, 예: Q13)로 환산한 표시값 — 엑셀 변수명·코딩북과 동일 기준. 질문코드가 없으면 제목의 Qx 파싱 폴백, 둘 다 없으면 공백이며 위치 서수("N번째")는 쓰지 않는다. 셀 단위 정밀도가 아니라 페이지 단위다(셀 단위 추적은 별도 응답 페이지 확장 필요).
