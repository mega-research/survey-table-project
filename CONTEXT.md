...
> 구현 상태: **인증 + 팀·멤버십 + 설문 귀속까지 구현** (2026-08-26 갱신 — v2 티켓 07 로 `surveys` 에 소유·배치 컬럼(0089)이 붙고, 접근 판정이 `server/survey-access.ts` 의 `resolveSurveyCapabilities` 하나로 모였다. 아래 어휘 중 **팀·팀 역할·시스템 전체 보기·팀 미배치 사용자·팀원 추가·배치 대기 설문·공개 범위**는 이제 코드의 설명이다. 아직 없는 것: 설문 그룹·참여자·해산·재배치(티켓 12·13·14·18). **관문 배선은 빌더·분석 도메인 + 콘솔 RSC(preview·분석·응답 상세 편집) + 설문 목록·생성까지다**(티켓 09, 2026-08-26) — 운영 콘솔·REST 는 종전 가드(계정 상태 + 게스트 grant)만 받으며 티켓 10·11 이 갈아끼운다). **참여자·게스트 절은 v1 어휘(editor/viewer·비계정 링크)로 남아 있다** — v2 는 참여자 단일화·게스트 계정 발급으로 바뀌었고(스펙 §0), 티켓 18·21 이 이 절을 갈아끼운다.
