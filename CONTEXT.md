...
Raw Data 시트와 분할 시트 왼쪽에 붙는 운영 메타 열 묶음 — 번호(systemID)(=resid)·순번·조사 대상 그룹·개별 URL·IP 해시·상태·시작일시·종료일시·소요시간·마지막 입력 문항·접속 단말. 번호(systemID)와 조사 대상 그룹은 설문 설정 기준 조건부 열이다 — 컨택 타겟이 없는 설문은 번호 열을, 그룹값이 전무한 설문은 그룹 열을 만들지 않는다(응답 매칭 여부 무관). 순번은 항상 존재한다. SPSS 변수가 아니므로 코딩북·.sav에는 포함하지 않으며 헤더는 3행 세로 병합이다.
...
**마지막 입력 문항 (last entered step)**:
진행 위치(currentStepId, 페이지 단위)를 그 페이지 대표(첫) 질문의 질문코드(questionCode, 예: Q13)로 환산한 표시값 — 엑셀 변수명·코딩북과 동일 기준. 진행 위치가 없거나(추적 도입 전 구응답) 현재 문항 구조와 매칭 실패하면 응답값이 실제 존재하는 질문 중 최후순 질문의 라벨로 폴백하고, 그래도 없으면 공백이다. 위치 서수("N번째")는 쓰지 않는다. 셀 단위 정밀도가 아니라 페이지 단위다(셀 단위 추적은 별도 응답 페이지 확장 필요).
_Avoid_: 마지막 응답 질문(질문 단위 어감), N번째 위치 표기
