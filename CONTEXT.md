...
> 구현 상태: **Better Auth 서버 기반만 구현, 나머지 미착수** (2026-08-25 갱신 — v2 티켓 01로 인증 5테이블 drizzle 스키마·`lib/auth/server.ts` 인스턴스·`users.user_type` 마이그레이션이 코드에 들어왔다. teams/workspace 스키마는 여전히 없다). 아래 어휘는 설계 합의(ADR 0006~0011 + 0018·0019)의 용어 정의이며 현재 코드의 설명이 아니다. 현행 운영 권한 모델은 admin allowlist + 게스트 설문 grant다 (CLAUDE.md "인증과 권한") — 세션 스왑은 티켓 02.
