...
응답 페이지가 좌(조사표) / 우(질문)로 갈라진 상태. **설정이 아니라 파생이다** — 토글이 없고, 앵커를 가진 항목이 처음 나타나는 페이지부터 마지막 페이지까지 유지된다(sticky). 판정은 스냅샷 구조 기준이라 조건부로 숨은 질문의 앵커도 세지만, 좌측에 사각형을 *그리는* 것은 표시되는 항목 것만이다. 레이아웃은 구조에서, 표시는 조건에서.
