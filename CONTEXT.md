...
Raw Data 시트와 분할 시트 왼쪽에 붙는 운영 메타 열 묶음 — 번호(resid)·순번·조사 대상 그룹·개별 URL·IP 해시·상태·시작일시·종료일시·소요시간·마지막 입력 문항·접속 단말. 번호와 순번은 모드 분기 없이 항상 함께 나간다. SPSS 변수가 아니므로 코딩북·.sav에는 포함하지 않으며 헤더는 3행 세로 병합이다.
_Avoid_: 식별자 모드(systemID/순번 택일 — 폐기된 개념), 메타 변수
