...
> 구현 상태: **인증 + 팀·멤버십까지 구현, 설문 귀속은 미착수** (2026-08-26 갱신 — v2 티켓 06 으로 `teams`·`team_members`·`team_lifecycle_events`(0088)와 팀 관리 화면(`/admin/teams`)이 들어왔다. 아래 어휘 중 **팀·팀 역할·시스템 전체 보기·팀 미배치 사용자·팀원 추가**는 이제 코드의 설명이다. 아직 없는 것: `surveys.team_id`·공개 범위·설문 그룹·참여자·해산·재배치(티켓 07·12·13·14·18). 현행 설문 접근은 여전히 계정 상태(active) + 게스트 설문 grant 다 — 팀은 설문 접근에 아직 관여하지 않는다). **참여자·게스트 절은 v1 어휘(editor/viewer·비계정 링크)로 남아 있다** — v2 는 참여자 단일화·게스트 계정 발급으로 바뀌었고(스펙 §0), 티켓 18·21 이 이 절을 갈아끼운다.
