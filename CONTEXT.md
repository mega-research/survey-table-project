...
Raw Data 시트와 분할 시트 왼쪽에 붙는 운영 메타 열 묶음 — IP 해시·번호(systemID)(=resid)·순번·조사 대상 그룹·개별 URL·상태·마지막 입력 문항·시작일시·종료일시·소요시간·접속 단말 순. 번호(systemID)와 조사 대상 그룹은 설문 설정 기준 조건부 열이다 — 컨택 타겟이 없는 설문은 번호 열을, 그룹값이 전무한 설문은 그룹 열을 만들지 않는다(응답 매칭 여부 무관). 순번은 항상 존재한다. SPSS 변수가 아니므로 코딩북·.sav에는 포함하지 않으며 헤더는 3행 세로 병합이다.
_Avoid_: 식별자 모드(systemID/순번 택일 — 폐기된 개념), 메타 변수, 빈 값 기준 열 생략(기준은 설문 설정)
