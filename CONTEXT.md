...
질문별 모바일 테이블 표현 정책. 자동 카드, 선택 행 원본 보기, 전체 원본 표 중 정확히 하나이며 여러 독립 토글의 조합이 아니다.
...
선택 행 원본 보기의 상세에서 반복하지 않을 원본 테이블 왼쪽 선행 열 개수. 현재 보이는 열이 아니라 작성된 열 순서를 기준으로 하며 기본값은 1이다.
