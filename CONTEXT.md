...
> 구현 상태: **인증만 구현, 워크스페이스는 미착수** (2026-08-26 갱신 — v2 티켓 01·02로 인증 5테이블 drizzle 스키마·`lib/auth/server.ts` 인스턴스·`users.user_type` 마이그레이션과 세션 스왑(로그인·미들웨어·REST 전부 Better Auth)이 들어왔다. teams/workspace 스키마는 여전히 없다). 아래 어휘는 설계 합의(ADR 0006~0011 + 0018·0019)의 용어 정의이며 현재 코드의 설명이 아니다. 현행 운영 권한 모델은 계정 상태(active) + 게스트 설문 grant다 (AGENTS.md "인증과 권한") — 팀 귀속·참여자는 티켓 06 이후.
