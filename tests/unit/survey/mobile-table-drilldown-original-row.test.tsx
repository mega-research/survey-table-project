      ...
      rows={[
        {
          id: 'ranking-row',
          label: '순위 행',
          cells: [{ id: 'ranking-label', type: 'text', content: '순위 행' }, rankingCell],
        },
      ]}
      mobileTableDisplayMode="drilldown-original-row"
      mobileDrilldownOmitLeadingColumns={1}
...
it.each([
  ...
  '범위 %s-%s의 헤더와 본문 순서를 적용한다',
  (start, end, showsHeader, repeatedLabels) => {
    ...
    const answer = screen.getByPlaceholderText('응답 행 입력');
    repeated.forEach((element) => {
      expect(
        element.compareDocumentPosition(answer) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
  },
);
...
it.each([
  ...
  '필드가 없는 과거 질문은 hideColumnLabels=%s의 기존 헤더 표시를 보존한다',
  (hideColumnLabels, showsHeader) => {
...
it('테스트 모드에서 반복행 preview는 store를 건드리지 않고 선택 행 응답을 기존 값에 병합한다', () => {
  useTestResponseStore.getState().updateTestResponse('repeat-header-test-question', {
    existing: 'keep',
  ...
  enterRepeatResponseRow();
  expect(screen.getByPlaceholderText('본문 1행 입력')).toBeDisabled();
  expect(screen.getByPlaceholderText('본문 1행 입력')).toHaveValue('');
  fireEvent.change(screen.getByPlaceholderText('응답 행 입력'), {
    target: { value: 'new-answer' },
  });

  expect(useTestResponseStore.getState().testResponses['repeat-header-test-question']).toEqual({
    existing: 'keep',
    'repeat-input-1': 'saved-repeat-value',
    'repeat-input-4': 'new-answer',
  });
});
...
it('현재 leaf 앞에 항목이 삽입되거나 재정렬되어도 row identity로 같은 상세를 유지한다', () => {
  ...
  rerender(
    <RerenderableShell
      sections={[
        section([leaf('inserted', '삽입 항목'), leaf('r1', '첫 항목'), leaf('r2', '둘째 항목')]),
      ]}
    />,
  );
...
it('현재 leaf가 제거되면 저장된 nav도 정리하고 해당 section의 안전한 목록으로 돌아간다', async () => {
  ...
  const { rerender } = render(<RerenderableShell sections={[original]} />);
  enterFirstLeaf();
  fireEvent.click(screen.getByRole('button', { name: '다음 항목' }));

  rerender(<RerenderableShell sections={[section([leaf('r1', '첫 항목')], { label: '척도' })]} />);
...
it('현재 section이 제거되면 저장된 nav도 정리하고 빈 화면 없이 안전한 root로 돌아간다', async () => {
  ...
  rerender(<RerenderableShell sections={[remaining, removed]} onReturnToRoot={onReturnToRoot} />);
  expect(screen.queryByTestId('rerender-leaf-detail')).toBeNull();
  expect(screen.getByText('작성할 항목을 선택하세요')).toBeInTheDocument();
  expect(onReturnToRoot).toHaveBeenCalledTimes(1);
});
