...
describe('PublicSurveyPreviewPage (/preview/[token])', () => {
  ...
  it('유효한 previewToken은 해당 설문을 resolve해 SurveyResponseFlow를 preview 모드로 렌더한다', async () => {
    getSurveyByPreviewTokenMock.mockResolvedValue({ id: SURVEY_ID });
    ...
    const result = await PublicSurveyPreviewPage({
      params: Promise.resolve({ token: PREVIEW_TOKEN }),
    });

    expect(getSurveyByPreviewTokenMock).toHaveBeenCalledWith({ token: PREVIEW_TOKEN });
    ...
    // surveyIdentifier 는 내부 UUID(survey.id) 가 아니라 previewToken 을 그대로 넘긴다 —
    ...
    // 노출할 이유가 없다.
    expect(flow?.props?.['surveyIdentifier']).toBe(PREVIEW_TOKEN);
  ...
  it('알 수 없는 token은 404 처리하고 이후 조회를 진행하지 않는다', async () => {
    getSurveyByPreviewTokenMock.mockResolvedValue(undefined);

    await expect(
      PublicSurveyPreviewPage({ params: Promise.resolve({ token: 'unknown-token' }) }),
    ).rejects.toThrow('NEXT_NOT_FOUND');

    expect(getSurveyByPreviewTokenMock).toHaveBeenCalledWith({ token: 'unknown-token' });
  ...
  it('형식이 깨진(malformed) token은 500 크래시 없이 404 처리한다', async () => {
    // getSurveyByPreviewToken(실 서비스)은 uuid 컬럼 비교 전에 형태를 검사해 undefined 를
    ...
    expect(getSurveyByPreviewTokenMock).toHaveBeenCalledWith({ token: malformedToken });
    expect(getSurveyByIdMock).not.toHaveBeenCalled();
    expect(getSurveyForResponseMock).not.toHaveBeenCalled();
  });

  it('token이 우연히 다른 설문의 id/slug와 같아도 resolve되지 않는다', async () => {
    // getSurveyByPreviewToken 은 previewToken 컬럼만 조회하는 실 서비스 함수이므로
    // (survey-read.service.ts 의 eq(surveys.previewToken, input.token) — id/slug 로는
    ...
    expect(getSurveyByPreviewTokenMock).toHaveBeenNthCalledWith(1, { token: SURVEY_ID });
    expect(getSurveyByPreviewTokenMock).toHaveBeenNthCalledWith(2, { token: SURVEY_ROW.slug });
    expect(getSurveyByIdMock).not.toHaveBeenCalled();
  });
  ...
  it('soft-delete된 설문은 404 처리한다', async () => {
    ...
    await expect(
      PublicSurveyPreviewPage({ params: Promise.resolve({ token: PREVIEW_TOKEN }) }),
    ).rejects.toThrow('NEXT_NOT_FOUND');
    expect(getSurveyForResponseMock).not.toHaveBeenCalled();
  });
