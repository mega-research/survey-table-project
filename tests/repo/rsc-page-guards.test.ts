...
/** 자기 가드로 인정하는 호출. 무엇을 막는지는 각 가드가 정한다(내부 전용·유형·설문 경계·탭). */
const PAGE_GUARD =
  /\b(requireAuth|requireAdminPage|requireSuperadminPage|requireActiveAccount|requireAccountTypePage|assertSurveyConsolePageAccess|assertGuestSurveyPageAccess|assertFieldworkSurveyPageAccess)\s*\(/;
