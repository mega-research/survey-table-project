...
const SURFACES: Record<string, SurfaceSpec> = {
  ...
  'contacts.idLists.create': {
    gate: 'scoped',
    capability: 'contacts.view',
    input: { surveyId: S, ids: [1] },
  },
  'contacts.priorAnswers.suggestMapping': {
    gate: 'rpc',
    capability: 'contacts.manage',
    input: { surveyId: S, file: xlsxFile() },
  },
  'contacts.priorAnswers.import': {
    gate: 'rpc',
    capability: 'contacts.manage',
    input: {
      surveyId: S,
  ...
  'contacts.priorAnswers.saveConfig': {
    gate: 'rpc',
    capability: 'survey.edit',
    input: { surveyId: S, blockMappings: {}, valueAliases: {} },
  },

  // ── 조사표 · 영역 앵커 ───────────────────────────────────────────────────
  'surveyDocument.documents.list': {
    gate: 'rpc',
    capability: 'survey.view',
    input: { surveyId: S },
  },
  'surveyDocument.documents.attach': {
    gate: 'rpc',
    capability: 'survey.edit',
    input: { surveyId: S, key: 'tmp/survey-document/x.pdf', filename: 'x.pdf', pageCount: 1 },
  },
  'surveyDocument.documents.remove': {
    gate: 'rpc',
    capability: 'survey.edit',
    input: { surveyId: S, documentId: C },
  },
  'surveyDocument.anchors.list': {
    gate: 'rpc',
    capability: 'survey.view',
    input: { surveyId: S },
  },
  'surveyDocument.anchors.create': {
    gate: 'rpc',
    capability: 'survey.edit',
    input: {
      surveyId: S,
      ...
      ownerKind: 'question',
      ownerId: C,
      rect: { page: 1, x: 0, y: 0, w: 0.1, h: 0.1 },
    },
  },
  'surveyDocument.anchors.remove': {
    gate: 'rpc',
    capability: 'survey.edit',
    input: { surveyId: S, anchorId: C },
  },
  ...
  'workspace.guests.searchCandidates': {
    gate: 'rpc',
    capability: 'survey.invite',
    input: { surveyId: S, query: '' },
  },
  'workspace.guests.add': {
    gate: 'rpc',
    capability: 'survey.invite',
    input: { surveyId: S, userId: C },
  },
...
const RESPONDENT_SURFACES = new Set([
  ...
  'quota.check',
  // 이월 응답 프리필 — 초대 토큰이 자격이다. 토큰·설문이 함께 맞아야 값을 주고 삭제된 설문은 null.
