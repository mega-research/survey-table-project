...
> **타 업체 초대 설문은 팀장에게도 NOT_FOUND**, 동료만 초대된 설문은 실사원에게 NOT_FOUND
> 이지만 팀장에게는 FORBIDDEN(존재는 인정). 그 대비가 요점이다: 갈리지 않으면 파생 시야가
> 무의미해지거나 업체 경계가 무의미해진다. scoped 인벤토리는 게스트 스위트와 **공용
> 헬퍼**(`tests/helpers/scoped-surface-inputs`)로 합쳤다 — 각자 표를 들면 새 표면이 붙었을
> 때 한쪽만 빨개진다. 마이그레이션 없음.
> **번호 재배치**: 이 브랜치의 12개가 0084~~0099 에서 **0101~0112** 로 밀렸다(main 과
> 0084·0085·0096·0099 가 같은 숫자로 다른 뜻이었다). 직전: 티켓 27 대리 응답 귀속)
>
> 티켓 27 **대리 응답 귀속 + 대행 배너** — 실사가
...
> 관리 표면 5종은 전부 슈퍼어드민 — 업체 목록은 협력사 명부라 팀장에게도 실사 팀장에게도
> 열지 않는다. **종료는 재직 중 계정이 0명일 때만** 되고, 종료된 업체의 소속 계정은 복귀·
> 재입사도 막는다: 실사 계정에는 「미배치」에 해당하는 상태가 없어(소속이 NOT NULL) 종료된
...
> (`capability-matrix-spec.test.ts`), 셀 단언만으로는 표가 자기 누락을 모르므로 완전성 가드
> 둘이 함께 선다 — 모든 capability 어휘가 정확히 한 행에 있고, 각 열의 판정 결과가 그 열의
> O 칸과 **정확히 같다**. **게스트 cross-survey 는 사유의 대비로 묻는다** — 부여=FORBIDDEN
> 對 미부여·남의 부여=NOT_FOUND. 갈리지 않으면 게스트 계정 하나로 조사 목록의 존재를 훑을
> 수 있다(목의 rowsFor 는 WHERE 를 모르므로 실 DB 다). soft delete 에 **참여자·게스트 축**을
> 더했다 — 팀 경계를 넘는 두 통로도 함께 닫히고 복구하면 부여가 그대로 돌아온다. export REST
> 가드가 `requireActiveAccount` 도 받고 있었는데(그 문은 계정 유형을 안 본다) `requireAuth`
> 로 조였다. 코드 변경은 그 한 줄뿐이고 나머지는 전부 테스트다. 마이그레이션 없음.
> 직전: 티켓 22 게스트 콘솔)
>
> 티켓 22 **게스트 콘솔** — 부여가 실제로 열리는 화면이
